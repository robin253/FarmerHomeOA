...
public final class DateTools {
	...
	public static List<String> getBeforeMonths(int n) {
		...
		for(int i=0;i<n;i++){
			...
			months.add(DateTools.yyyymmFormat.format(cal.getTime()));
		}
		return months;
	}
	...
	public static List<String> getMiddleMonths(String startTime,String endTime){
		...
		try {
			...
			beforeCalendar.setTime(DateTools.datetimeFormat.parse(startTime));
			afterCalendar.setTime(DateTools.datetimeFormat.parse(endTime));
			DateTools.toLastDateOfMonth(afterCalendar);
			while(beforeCalendar.compareTo(afterCalendar) <= 0){
				list.add(DateTools.yyyymmFormat.format(beforeCalendar.getTime()));
	...
	public static String addWorkDay(Calendar c,int add){
		...
		return DateTools.datetimeFormat.format(c.getTime());
	}
