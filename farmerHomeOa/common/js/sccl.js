...
function addIframe(cur){
	...
	if (!isHas) {
		var tab = "<a href='javascript:;' class='content-tab active' data-id='"+h+"'>"+ label +" <i class='iconfont'>&#xe63e;</i></a>";
	...
	for(var i=0; i<menu.length; i++){   
		...
		try{
			if(item.isHeader == "1"){
				...
				if(item.childMenus == ""){
				      str = "<li><a href='"+item.url+"'><i class='iconfont'>"+item.icon+"</i><span>"+item.name+"</span></a></li>";
					  $(parent).append(str);
				}else{
					str = "<li><a href='"+item.url+"'><i class='iconfont'>"+item.icon+"</i><span>"+item.name+"</span></a>";
...
//$(window.frames.document).contents().find("#layout-skin").attr("href","molv");	
//	("scclui-skin", "molv");
